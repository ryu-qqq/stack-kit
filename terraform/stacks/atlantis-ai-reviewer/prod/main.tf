...
# ALB HTTPS Listener (only create if not using existing ALB and certificate is provided)
resource "aws_lb_listener" "atlantis_https" {
  count = var.use_existing_alb ? 0 : (var.certificate_arn != "" ? 1 : 0)
...
# ALB HTTP Listener (fallback when no certificate)
resource "aws_lb_listener" "atlantis_http_fallback" {
  count = var.use_existing_alb ? 0 : (var.certificate_arn == "" ? 1 : 0)
...
# Only creates if both domain_name and hosted_zone_id are provided
# Otherwise, users can access Atlantis directly via ALB DNS name
resource "aws_route53_record" "atlantis" {
  ...
  alias {
    name                   = var.use_existing_alb ? var.existing_alb_dns_name : aws_lb.atlantis[0].dns_name
    zone_id                = var.use_existing_alb ? data.aws_lb.existing_atlantis[0].zone_id : aws_lb.atlantis[0].zone_id
    evaluate_target_health = true
  }
}

# === ECR Repository for Atlantis === (Temporarily commented for destroy)
# module "atlantis_ecr" {
#   source = "../../modules/ecr"
#   
#   repository_name = "${var.project_name}-${var.env}-atlantis"
...
# === Docker Image Build and Push === (Temporarily commented for destroy)
# resource "null_resource" "atlantis_docker_build" {
#   # Trigger rebuild when Dockerfile or entrypoint changes
#   triggers = {
#     dockerfile_hash = filemd5("${path.root}/../../../atlantis/Dockerfile")
#     entrypoint_hash = filemd5("${path.root}/../../../atlantis/docker-entrypoint.sh")
#     timestamp = timestamp()
#   }
# 
#   provisioner "local-exec" {
#     command = <<-EOT
#       cd ${path.root}/../../../atlantis
#       
#       # Get ECR login token
#       aws ecr get-login-password --region ${var.region} | docker login --username AWS --password-stdin ${module.atlantis_ecr.repository_url}
#       
#       # Build the image
#       docker build -t stackkit-atlantis:latest .
#       
#       # Tag for ECR with version and latest
#       IMAGE_TAG="v$(date +%Y%m%d-%H%M%S)"
#       docker tag stackkit-atlantis:latest ${module.atlantis_ecr.repository_url}:$IMAGE_TAG
#       docker tag stackkit-atlantis:latest ${module.atlantis_ecr.repository_url}:latest
#       
#       # Push to ECR
#       docker push ${module.atlantis_ecr.repository_url}:$IMAGE_TAG
#       docker push ${module.atlantis_ecr.repository_url}:latest
#       
#       # Output the image tag for reference
#       echo "Built and pushed image with tag: $IMAGE_TAG"
#       
#       echo "Successfully built and pushed Atlantis image to ECR"
#     EOT
#   }
# 
#   depends_on = [module.atlantis_ecr]
# }
...
output "atlantis_url" {
  ...
  value = var.domain_name != "" ? (
    ...
    var.certificate_arn != "" ? "https://${var.use_existing_alb ? var.existing_alb_dns_name : aws_lb.atlantis[0].dns_name}" : "http://${var.use_existing_alb ? var.existing_alb_dns_name : aws_lb.atlantis[0].dns_name}"
  )
}

output "alb_dns_name" {
  description = "ALB DNS name (use this if no custom domain is configured)"
  value = var.use_existing_alb ? var.existing_alb_dns_name : aws_lb.atlantis[0].dns_name
}

# output "ecr_repository_url" {
#   description = "ECR repository URL for Atlantis images"
