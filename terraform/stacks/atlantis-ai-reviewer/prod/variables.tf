...
# Leave these empty to use ALB DNS name directly without custom domain
variable "domain_name" {
  description = "Optional: Custom domain name for Atlantis (e.g., atlantis.example.com). Leave empty to use ALB DNS name."
  type        = string
  default     = ""
}

variable "certificate_arn" {
  description = "Optional: ARN of the SSL certificate for HTTPS. Leave empty to use HTTP only."
  type        = string
  default     = ""
}

variable "hosted_zone_id" {
  description = "Optional: Route53 hosted zone ID for the domain. Required only if domain_name is provided."
  type        = string
  default     = ""
}
