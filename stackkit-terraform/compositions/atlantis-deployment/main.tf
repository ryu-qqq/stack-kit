...
resource "aws_security_group" "alb" {
  ...
  ingress {
    ...
    description = "HTTP access from ${length(var.allowed_cidr_blocks) > 0 ? "allowed IPs" : "anywhere"}"
  }
  ...
  ingress {
    ...
    protocol    = "tcp"
    cidr_blocks = length(var.allowed_cidr_blocks) > 0 ? var.allowed_cidr_blocks : ["0.0.0.0/0"]
    description = "HTTPS access from ${length(var.allowed_cidr_blocks) > 0 ? "allowed IPs" : "anywhere"}"
  }
